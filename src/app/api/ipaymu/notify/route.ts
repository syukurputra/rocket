...
/**
 * Tagihan yang cocok dengan referenceId dari iPaymu.
 *
 * Ada dua bentuk referenceId yang dipakai:
 * - booking satuan  → id tagihan (dikirim berawalan "BKG-")
 * - bayar keranjang → orderId, dipakai bersama oleh beberapa tagihan sekaligus
 */
function findTagihanByReference(referenceId: string) {
  return prisma.tagihan.findMany({
    where: {
      OR: [{ id: referenceId.replace(/^BKG-/, '') }, { orderId: referenceId }]
    },
    include: {
...
type TagihanNotify = Awaited<ReturnType<typeof findTagihanByReference>>[number]
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ─── Cari tagihan dulu by ID/orderId, lalu invoice ───────────────────────
    const daftarTagihan = await findTagihanByReference(referenceId)

    if (daftarTagihan.length > 0) {
      console.log(`[iPaymu Notify] Tipe: TAGIHAN — ${daftarTagihan.length} tagihan cocok`)

      // Pembayaran dari keranjang melunasi beberapa tagihan sekaligus
      for (const tagihan of daftarTagihan) {
        if (ipaymuStatus === 'berhasil' && tagihan.status !== 'LUNAS') {
          await prosesTagihanLunas(tagihan)
        } else {
          console.log(`[iPaymu Notify] Tagihan "${tagihan.id}" status "${ipaymuStatus}" — tidak ada aksi`)
        }
      }
    ...
    return await prosesInvoice(referenceId, ipaymuStatus, rawStatus, trxId)
  } catch (error) {
...
/** Tandai satu tagihan LUNAS, catat pendapatan/keuangan, lalu kabari penyewa & pemilik. */
async function prosesTagihanLunas(tagihan: TagihanNotify) {
...
async function prosesInvoice(
  referenceId: string,
  ipaymuStatus: ReturnType<typeof parseIpaymuStatus>,
  rawStatus: string,
  trxId: string
) {
